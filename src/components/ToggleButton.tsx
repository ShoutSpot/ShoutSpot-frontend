...
export const ToggleButton = () => {
    ...
    return (
        <>
            <button type="button" aria-pressed={isActive} className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 outline-none  ${isActive ? 'bg-purple-600' : 'bg-gray-300'}`} onClick={toggleState}>
                <span className={`relative inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 ${isActive ? 'translate-x-5' : 'translate-x-0'}`}>
                    ...
                    <span className={`absolute inset-0 h-full w-full flex items-center justify-center transition-opacity ${isActive ? 'opacity-100' : 'opacity-0'} ease-out duration-100`} aria-hidden="true">
